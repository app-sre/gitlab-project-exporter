...
container:
	$(CONTAINER_ENGINE) build -t $(IMAGE_NAME):release $(foreach arg,$(BUILD_ARGS),--build-arg $(arg)) .
.PHONY: container

container-test:
	$(CONTAINER_ENGINE) build --target test -t $(IMAGE_NAME):test $(foreach arg,$(BUILD_ARGS),--build-arg $(arg)) .
